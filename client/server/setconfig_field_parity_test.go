...
var setConfigRequestFieldCases = map[string]setConfigRequestFieldCase{
	...
	// GetConfigResponse does not report the local metrics endpoint's
	// current state (see config_diff.go), so SetConfigRequestChangesConfig
	// treats any explicit override as a change; no prepareCur is needed to
	// make the change observable.
	"EnableLocalMetrics":  {req: &proto.SetConfigRequest{EnableLocalMetrics: boolPtr(true)}},
	"LocalMetricsAddress": {req: &proto.SetConfigRequest{LocalMetricsAddress: stringPtr("127.0.0.1:9090")}},

	"RemoteJobsAllowed": {req: &proto.SetConfigRequest{RemoteJobsAllowed: boolPtr(true)}},
}
