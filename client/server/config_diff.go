...
func SetConfigRequestChangesConfig(msg *proto.SetConfigRequest, current *proto.GetConfigResponse) bool {
	...
	// Compare both sides as the endpoints they address, not as strings: msg
	...
	// that was never actually changed. util.SameServiceURLStrings is also
	// what mdm.ConflictURL uses for the equivalent MDM comparison.
	if msg.ManagementUrl != "" && !util.SameServiceURLStrings(msg.ManagementUrl, current.ManagementUrl) {
		return true
	}
	if msg.AdminURL != "" && !util.SameServiceURLStrings(msg.AdminURL, current.AdminURL) {
		return true
	}
	...
	// EnableLocalMetrics/LocalMetricsAddress: GetConfigResponse does not
	// report the local metrics endpoint's current state, so - like
	// OptionalPreSharedKey above - there is no current value to diff
	// against. Treat any explicit override as a change.
	if msg.EnableLocalMetrics != nil || msg.LocalMetricsAddress != nil {
		return true
	}
	...
	return boolPtrChanged(msg.RosenpassEnabled, current.RosenpassEnabled) ||
		...
		boolPtrChanged(msg.RemoteJobsAllowed, current.RemoteJobsAllowed)
}
